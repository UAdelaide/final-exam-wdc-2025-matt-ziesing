...
app.get('/api/walkrequests/open', async (req, res) => {
  try {
    const [dogList] = await db.execute('SELECT request_id, name, requested_time, duration_minutes, location, username FROM WalkRequests w JOIN Dogs ON .owner_id = Users.user_id');
    res.json(dogList);
  } catch (err) {
    res.status(500).json({ error: 'Failed to fetch books' });
  }

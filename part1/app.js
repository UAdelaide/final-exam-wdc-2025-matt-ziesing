...
app.get('/api/walkrequests/open', async (req, res) => {
  try {
    const [dogList] = await db.execute('SELECT request_id, name, requested_time, duration_minutes, location, username FROM WalkRequests w JOIN Dogs d ON w.dog_id = d.dog_id JOIN Users u ON ');
    res.json(dogList);
  } catch (err) {
    res.status(500).json({ error: 'Failed to fetch books' });
  }
